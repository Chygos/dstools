...
# SHAP 
def get_shap_values(model, df, return_dataframe=True):
    """
    ...
    shap_vals = shap_vals[..., 1] if len(shap_vals.shape) == 3 else shap_vals
    if return_dataframe:
        shap_val_mean = np.abs(shap_vals.values).mean(0) if hasattr(shap_vals, 'values') else np.abs(shap_vals).mean(0)
        features = df.columns.tolist() if isinstance(df, pd.DataFrame) else [f'X{i}' for i in df.shape[1]]
        varimp = pd.Series(shap_val_mean, features)
        return varimp
    else: 
        return shap_vals
...
def plot_shap_feature_importance(shap_values:Union[shap._explanation.Explanation, np.ndarray], 
                                 feature_names, topn=25, title=None):
